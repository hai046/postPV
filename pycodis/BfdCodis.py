...
# !/usr/bin/env python
# encoding:utf-8
# https://github.com/baifendian/CodisClient
import logging
...
class BfdCodis(object):
    def __init__(self, zkAddr, proxyPath):
    ...
    def __watcher(self, event):
        ...
        if event.type == "SESSION" and event.state == "CONNECTING":
            ...
            logger.error("zookeeper connection state changed but not implemented: event:%s state:%s path:%s" % (
